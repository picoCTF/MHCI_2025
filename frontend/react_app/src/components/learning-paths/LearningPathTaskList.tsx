...
const LPTaskInfo: React.FC<LearningPathTaskInfoProps> = ({task}) => {
    ...
    const { data: taskContentInfo, isLoading: taskContentInfoLoading } = useMockData<LPChallengeInfo | null>(null);

    if(!taskContentInfoLoading) {
        if(task.contentType == "C" && taskContentInfo) {
            return (
                <div className="flex flex-row gap-4 items-center">
                    <div className="flex row gap-2 items-center">
                        <Icon icon={"material-symbols:star-outline"} width={20} height={20} className={"text-default-500"}/>
                        <p className="flex font-mono text-default-500">{(taskContentInfo as LPChallengeInfo).difficulty}</p>
                    </div>
            ...
            <div className="flex flex-col w-full h-fit">
                {
                    pagedTasks.results.map((item) => (
                        <Button className="flex flex-row w-full min-w-full h-[100%] pl-6 pr-3 py-2 gap-3 justify-start items-center" 
                            ...
                            <div className={"flex min-w-12 min-h-12 bg-" + (item.status == "Complete" ? "success" : "secondary") + "-50 rounded-full items-center place-content-center"}>
                                <Icon icon={(item.status == "Complete" ? "material-symbols:check-outline" : "material-symbols:book-outline")} width={20} height={20} className={"text-" + (item.status == "Complete" ? "success" : "secondary") + "-500"}/>
                            </div>
